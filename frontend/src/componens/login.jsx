...
const Login = () => {
  ...
  // Border Highlights based on Selected Active Node
  ...
  return (
    <div className="bg-slate-50 min-h-screen flex items-center justify-center p-4 md:p-0 font-sans relative overflow-hidden transition-colors duration-500">
      ...
      {/* Main Container Card (With Dynamic Role Hover Shadows) */}
      <div className={`bg-white/90 backdrop-blur-xl flex rounded-[2rem] shadow-xl border border-slate-100 max-w-4xl w-full min-h-[560px] overflow-hidden relative z-10 transition-all duration-500 ${getCardActiveBorder()}`}>
        
        {/* ================= LEFT PANEL (MEDCARE OS) WITH POP EFFECTS ================= */}
        <div className="hidden md:flex md:w-1/2 bg-slate-900 p-12 text-white flex-col justify-between relative group/sidebar overflow-hidden">
          ...
          {/* Neon inner ambient spotlight (follows sidebar hover) */}
          ...
          {/* 1. Brand Logo: Elastic Pop & Rotate Effect */}
          ...
          {/* 2. Heading Section: Text Pop & Smooth Slide Underline */}
          <div className="z-10 space-y-5 group/text cursor-default">
            ...
            <div className="relative inline-block">
              <h1 className="text-3xl font-black leading-tight tracking-tight text-slate-100 transition-all duration-500 group-hover/text:scale-[1.02] group-hover/text:text-white origin-left">
                Digitalizing Healthcare, Elevating Care.
              </h1>
        ...
        {/* ================= RIGHT PANEL FORM (STAYS PERFECT) ================= */}
        <div className="w-full md:w-1/2 p-8 sm:p-12 flex flex-col justify-center bg-white">
          <div className="mb-6">
            <h2 className="text-2xl font-extrabold text-slate-800">System Sign In</h2>
            <p className="text-xs text-slate-400 mt-1">Please authenticate your department node identity</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide pl-0.5">Select Role</label>
              <div className={`relative bg-slate-50 border border-slate-200/80 rounded-xl transition-all duration-300 shadow-sm hover:scale-[1.03] hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md ${getFocusBorder()}`}>
                <select 
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="w-full px-4 py-3.5 bg-transparent outline-none text-sm font-medium text-slate-700 cursor-pointer appearance-none relative z-10"
                >
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs z-0 pointer-events-none">
                  ▼
                </div>
              </div>
            </div>
            ...
            {/* Persistent Cookie Session checkbox */}
